 ...
 *  Heavily modified by Kenny Hoff, 1998.
 *  email: ndr@pobox.com
 ...
 */
static GLvoid
_glmWriteMTL(GLMmodel* model, const char* modelpath, char* mtllibname)
{
  FILE* file;
  char* dir;
  char* filename;
...
/* glmExtents: Calculates the Min/Max extents of the model
 *
 * model      - initialized GLMmodel structure
 * Min/Max - array of 3 GLfloats (GLfloat dimensions[3]) 
 */
GLvoid glmExtents(GLMmodel* model, GLfloat* Min, GLfloat* Max)
{
  GLuint i;
  ...
  /* get the max/mins */
  Max[0] = Min[0] = model->vertices[3 + X];
  Max[1] = Min[1] = model->vertices[3 + Y];
  Max[2] = Min[2] = model->vertices[3 + Z];
  for (i = 1; i <= model->numvertices; i++) {
    if (Max[0] < model->vertices[3 * i + X])
      Max[0] = model->vertices[3 * i + X];
    if (Min[0] > model->vertices[3 * i + X])
      Min[0] = model->vertices[3 * i + X];

    if (Max[1] < model->vertices[3 * i + Y])
      Max[1] = model->vertices[3 * i + Y];
    if (Min[1] > model->vertices[3 * i + Y])
      Min[1] = model->vertices[3 * i + Y];

    if (Max[2] < model->vertices[3 * i + Z])
      Max[2] = model->vertices[3 * i + Z];
    if (Min[2] > model->vertices[3 * i + Z])
      Min[2] = model->vertices[3 * i + Z];
  }
}
...
GLvoid glmDraw(GLMmodel* model, GLuint mode)
{
  ...
  {
    // PER-GROUP MATERIAL SETTINGS (OR JUST A COLOR SETTING BASED ON DIFFUSE)
    if (mode & GLM_MATERIAL) 
    {
    ...
    // RENDER EACH TRIANGLE (USE THE APPROPRIATE DISPLAY ROUTINE)
...
/* glmGetTris : stuff all triangles into a flat array of floats; 9 floats
 * per tri.
 */
void glmGetTris(GLMmodel* model, float* &tris, int &numtris)
{
  assert(model);
  assert(model->vertices);

  // ALLOC ARRAY ONLY IF tris==NULL; OTHERWISE, MUST HAVE BEEN PREALLOCED
  numtris = glmGetNumTris(model);
  ...
  for (GLMgroup *group=model->groups; group; group=group->next) 
    ...
    {
      V = &( model->vertices[ 3 * model->triangles[ group->triangles[i] ].vindices[0] ] );
      T[k] = V[0];  k++;
      T[k] = V[1];  k++;
      T[k] = V[2];  k++;
      V = &( model->vertices[ 3 * model->triangles[ group->triangles[i] ].vindices[1] ] );
      T[k] = V[0];  k++;
      T[k] = V[1];  k++;
      T[k] = V[2];  k++;
      V = &( model->vertices[ 3 * model->triangles[ group->triangles[i] ].vindices[2] ] );
      T[k] = V[0];  k++;
      T[k] = V[1];  k++;
      T[k] = V[2];  k++;
    }
...
/* glmGetTris : stuff all triangles into a flat array of doubles; 9 doubles
 * per tri.
 */
void glmGetTris(GLMmodel* model, double* &tris, int &numtris)
{
  assert(model);
  assert(model->vertices);
...
int glmGetNumTris(GLMmodel* model)
{
  // COUNT NUMBER OF TRIS IN MODEL
  int NumTris=0;
  GLMgroup* group;
  for (group=model->groups; group; group=group->next) 
    NumTris += group->numtriangles;
  return(NumTris);
}
