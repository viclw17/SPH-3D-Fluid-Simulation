...
template<class Type>
Mat44<Type>& Mat44<Type>::operator *= (const Mat44& A)         // ACCUMULATE MATRIX MULTIPLY (*=)
{
...
template<class Type>
Type Mat44<Type>::Determinant() const
{
...
template<class Type>
bool Mat44<Type>::Inverse(Mat44<Type> &inv, Type tolerance) const
{
  Type mSub[3][3] ;
  int a, b;
  ...
  for (int i = 0 ; i < 4 ; i++) 
  {
    for (int j = 0 ; j < 4 ; j++) 
    {
      ...
      inv.M[j + 4 * i] = mSub[0][0]*(mSub[1][1]*mSub[2][2] - mSub[2][1]*mSub[1][2]) 
	                     - mSub[0][1]*(mSub[1][0]*mSub[2][2] - mSub[2][0]*mSub[1][2])
	                     + mSub[0][2]*(mSub[1][0]*mSub[2][1] - mSub[2][0]*mSub[1][1]);
      
      if ((fabs(inv.M[j + 4 * i]) > tolerance) && ((i + j) % 2 != 0)) 
        inv.M[j + 4 * i] *= -1.0;
    }
  }
...
void Mat44<Type>::Rotate(Type DegAng, const Vec3<Type>& Axis)
{
  ...
  else                                      // ARBITRARY AXIS
  {
   ...
   {
     // needs normalization
     l=static_cast<Type>(1.0) / static_cast<Type>(sqrt(l));
     x*=l; y*=l; z*=l;
   }
