...
{
#if 0
  // Yick!  This uses 28 mults!  (+4 mul +1 div  for re-normalization)
  /* copy vector into temp quaternion for multiply	*/
  Quat  vecQuat(vec);
  /* invert multiplier	*/
  Quat  inverse(*this);

  Type scale = Type(1.0)/LengthSqr();
  inverse.v *= -scale;
  inverse.s *= scale;
...
#else
  // This version basically uses the ToMat code and does
  // the mat-vec mult.  This is much slimmer than the above code
  // (which does silly things like multiplying by zero a bunch)
  // It also uses the fact that x*x+y*y+z*z+s*s == 1 so that, e.g.
  // x2-y2-z2+s2 = 1-2y2-2z2, to save a few mults overall.
  // The upshot is you save mults (uses 21, +4mult +1div for renorm)
  // (If there's a faster way I can't see it)
  Type t, xs, ys, zs, wx, wy, wz, xx, xy, xz, yy, yz, zz;
  qvec ret;
  
  t  = Type(2.0) / (v.x*v.x+v.y*v.y+v.z*v.z+s*s); // good to be cautious
  const Type ONE(1.0);
  ...
  ret.x = vec.x*(ONE-yy-zz) + vec.y*(xy-wz)     + vec.z*(xz+wy);
  ret.y = vec.x*(xy+wz)     + vec.y*(ONE-xx-zz) + vec.z*(yz-wx),       
  ret.z = vec.x*(xz-wy)     + vec.y*(yz+wx)     + vec.z*(ONE-xx-yy);
...
template<class Type>
inline Type Quat<Type>::FUDGE()    { return Type(1e-6); }
template<>
